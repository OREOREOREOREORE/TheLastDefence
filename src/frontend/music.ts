...
class SoundManager {
    ...
    private musicBuffers: Map<string, AudioBuffer> = new Map<string, AudioBuffer>();
    ...
    setMusicVolume(volume: number): void{ this.musicGain.gain.value = volume;}
    setSfxVolume(volume: number): void{ this.sfxGain.gain.value = volume;}
    setMuted(muted: boolean): void {this.master.gain.value = muted ? 0 : 1;}
